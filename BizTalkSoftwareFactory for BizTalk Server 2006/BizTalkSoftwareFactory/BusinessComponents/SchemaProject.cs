    ...
    class SchemaProject
    {
        ...
        private static void GetSchemasFromReferences(Project project, List<SchemaItem> schemaList)
        {
            ...
            try
            {
                ...
                IBtsProject btsProject = (IBtsProject)project.Object;
                if (btsProject == null)
                {
                    return;
                }
                ...
                for (int i = 1; i <= ((int)((IBtsReferences)btsProject.References).Count); i++)
                {
                    // Get the path of the assembly and determine if it is a BizTalk assembly that contains Schemas
                    string assemblyPath = ((IBtsReference)((IBtsReferences)btsProject.References).Item(i)).Path;
        ...
        private static bool IsBizTalkSchemaType(Type assemblyType)
        {
            ...
            if (assemblyType.BaseType == typeof(SchemaBase) && !assemblyType.IsNestedPublic)
            {
                retVal = true;
            }
            else
